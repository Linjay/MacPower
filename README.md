...
当前版本：**MacPower 0.1.0 预览版**，按选定的方案 1 · Power Path 实现原生菜单栏程序，读取真实设备数据。支持电源输入、电池充放电、整机耗电、健康度与循环次数，包含历史曲线、CSV 导出，以及深色、浅色、跟随系统三种外观。

## 使用原生应用

从 [Release v0.1.0](https://github.com/Linjay/MacPower/releases/tag/v0.1.0) 下载 `MacPower-0.1.0-arm64.zip`，解压后双击 `MacPower.app`。此构建适用于 Apple Silicon Mac；源码最低部署目标为 macOS 14，实际验证环境是 macOS 27.0。

菜单栏默认显示电池净功率：正数充电，负数放电。点击数字打开面板，齿轮进入设置；右键菜单可打开独立概览窗口或退出。程序运行时再次打开应用也可显示概览窗口。
...
- 23 项自动测试通过；有效实机字段对照 32 项通过，3 项因采样间数据变化不作判定。
...
1. 菜单栏默认固定显示**电池净功率**：`+27.6 W` 表示充入电池，`−12.4 W` 表示从电池取电。用户可以主动改为电源输入或整机耗电，程序不自动偷换指标。
